...
FOMC_DECISION_FILE = os.path.join(DATA_DIR, "fomc_decisions_df.csv")
ASSET_DF_FILE = os.path.join(ROOT_DIR, DATA_DIR, "assets_df.csv")
...
if __name__ == "__main__":
    ...
    fomc_df = (
        fomc_df[fomc_df["Date"] > START_DATE]
    ...
    for i, date in enumerate(fomc_dates):
        ...
        t_1yr_change = (
            assets_df["FCM1"].loc[assets_df["Date"] == date].item()
            - assets_df["FCM1"].loc[assets_df["Date"] == last_fomc].item()
        )
        t_5yr_change = (
            assets_df["FCM5"].loc[assets_df["Date"] == date].item()
            - assets_df["FCM5"].loc[assets_df["Date"] == last_fomc].item()
        )
        t_10yr_change = (
            assets_df["FCM10"].loc[assets_df["Date"] == date].item()
            - assets_df["FCM10"].loc[assets_df["Date"] == last_fomc].item()
        )
        t_30yr_change = (
            assets_df["FCM30"].loc[assets_df["Date"] == date].item()
            - assets_df["FCM30"].loc[assets_df["Date"] == last_fomc].item()
        )
        implied_change = (
            assets_df["PFNP"].loc[assets_df["Date"] == date].item()
            - assets_df["PFNP"].loc[assets_df["Date"] == last_fomc].item()
        )
        sp500_change = (
            assets_df["SP500"].loc[assets_df["Date"] == date].item()
            - assets_df["SP500"].loc[assets_df["Date"] == last_fomc].item()
        )
        sp500f_change = (
            assets_df["SPSPF"].loc[assets_df["Date"] == date].item()
            - assets_df["SPSPF"].loc[assets_df["Date"] == last_fomc].item()
        )
