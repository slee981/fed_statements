...
def get_num_topics_per_period(num_topics, alpha, eta):
    ...
    topics_dist_per_section = [
        [0 for i in range(num_topics)] for i in range(len(corpus))
    ]
    topics_per_section = []
    dates = df["Date"].copy()

    print("\nFitting model with {} topics...".format(num_topics))
    ...
    for i, doc in enumerate(lda_model[corpus]):
        for topic_num, pct_topic in doc:
...
if __name__ == "__main__":
    ...
    for n in NUM_TOPICS:
        ...
        topics_per_period = get_num_topics_per_period(
            num_topics=n, alpha=alpha, eta=eta
        )
